...
client.ayarlar = {
  token:"ODU5ODU1MDE5MjA4MzQzNTk0.YNywYw.J8bFm1Un9ZrQcJm1s1PIU7lvzvk",
  prefix:"!",
  sahip: ["574230824334000129"]
  
}
client.commands = new Discord.Collection();
client.aliases = new Discord.Collection();
// --------------------------- Mod Komut Kategori ----------------------------
fs.readdir("./komutlar/mod/", (err, files) => {
  if (err) console.error(err);
  console.log(`${files.length} komut yüklenecek.`);
  files.forEach(f => {
    let props = require(`./komutlar/mod/${f}`);
...
client.reload = command => {
  return new Promise((resolve, reject) => {
    try {
      delete require.cache[require.resolve(`./komutlar/mod/${command}`)];
      let cmd = require(`./komutlar/mod/${command}`);
...
client.reload = command => {
  return new Promise((resolve, reject) => {
    try {
      delete require.cache[require.resolve(`./komutlar/yetkili/${command}`)];
      let cmd = require(`./komutlar/yetkili/${command}`);
      client.commands.delete(command);
      client.aliases.forEach((cmd, alias) => {
        if (cmd === command) client.aliases.delete(alias);
      });
...
let antikanal = db.fetch(`botkorumakanal_${member.guild.id}`)
member.kick(member)
if(antikanal) {
 var embed = new Discord.MessageEmbed()
 .setColor("GREEN")
.setDescription(`**Sunucuya Bir Bot Eklendi Anti Raid Sistemi Aktif Olduğundan Bot Atıldı. Botu Sunucuya Sokmak İçin !!anti-raid izin botunid**`)
